...
class RecipesProviderWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(create: (context) => RecipesModel(),
        child: const Recipes());
  }
}
...
class Recipes extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('Course').where("id", isEqualTo:"HgpJgQ9FppebnBmiuvNIbgKgiMG3").snapshots(),
      ...
      return showDialog<void>(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            actions: <Widget>[
              TextButton(
                  child: const Text('Да', style: TextStyle(color: Colors.black, fontSize: 15)),
                  onPressed: () {
                    Navigator.of(context).pop();
                  }
              ),
              TextButton(
                child: const Text('Нет', style: TextStyle(color: Colors.black, fontSize: 15)),
                onPressed: () {
                  Navigator.of(context).pop();
                },
              ),
    ...
    return Card(
      child: ListTile(
          ...
          onTap: () {Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const RecipesProviderWidget()),
          );},
          leading: ClipRRect(
